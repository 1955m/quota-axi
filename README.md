...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:cursor`, `pi:kimi-coding`, `pi:xai`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
- It never retains, prints, logs, renders, caches, sends, or exchanges a refresh token's value. A Pi credential broker may inspect only the property's shape or presence to derive a usability boolean and discards that fact with the in-memory reading; elsewhere only presence is checked as evidence that the vendor can still recover.
