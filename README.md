...
- The access token value is used only as the bearer of Cursor's read-only dashboard RPCs (`GetCurrentPeriodUsage`, `GetPlanInfo`, and `GetSandUsageStatus`) and optional first-party `/auth/full_stripe_profile` identity read. It is never logged, hashed, cached, or included in output. A stable account/customer identifier returned by these first-party responses is exposed only in `--full` as verified account identity; email alone is unverified. A failed live Cursor read stays failed and never serves a cached snapshot, even when a concurrent identity response names the same account, so switching between editor, CLI, and Pi can never serve another account's windows. quota-axi does not call Grok Bot trial, banked-reset, or machine-registration methods.
...
| Cursor cache provenance                | Cursor has no stale fallback: a failed live read stays failed and never serves a cached snapshot, even when a concurrent identity response names the same account.                                                                                                                                                                                                                                                                                                                                                                                                           |
